...
def pair(csv_path,man_set_label,woman_set_label):
    priorities = read_priorities(csv_path) 
    males = priorities[man_set_label]
    females = priorities[woman_set_label]
    ...
    while resume:
        for male in remaining_males:
            #male wants to match with top unrejected female
            desired_match = priorities[man_set_label][male][0]
            if(desired_match in proposed_couples.values()):
                keys = (proposed_couples.keys())
                for male_match in list(keys): #have to cast to list so its not a deep copy
                    if(proposed_couples[male_match] == desired_match):
                        #this female is already matched, check who they prefer more
                        cur_pref = females[desired_match].index(male_match)
                        proposed_pref = females[desired_match].index(male)
                        #female should switch
                        if(proposed_pref < cur_pref):
                            del proposed_couples[male_match]
                            proposed_couples[male] = desired_match
                        #female should not switch - remove female from options 
                        else:
                            del priorities[man_set_label][male][0]
            else:
                proposed_couples[male] = desired_match


        remaining_males = copy.copy(males) #must ensure shallow copy
        for male in proposed_couples.keys():
            del remaining_males[male]
        if(len(remaining_males) <= 0):
            resume = False
    return proposed_couples
